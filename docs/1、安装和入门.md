...
- [安装](#安装)
- [创建你的第一个测试用例](#创建你的第一个测试用例)
- [执行多个测试用例](#执行多个测试用例)
- [触发一个指定异常的断言](#触发一个指定异常的断言)
- [在一个类中组织多个测试用例](#在一个类中组织多个测试用例)
- [申请一个唯一的临时目录](#申请一个唯一的临时目录)
...
2. 查看安装的版本信息：
    ...
    ```bash
    λ pipenv run pytest --version
    This is pytest version 5.1.3, imported from c:\users\luyao\.virtualenvs\pytest-chinese-doc-bttvxdxm\lib\site-packages\pytest.py
    ```

3. 或者在项目的根目录中执行：`pipenv install --ignore-pipfile`；
...
# 执行多个测试用例
执行`pipenv run pytest`命令，它会执行当前及其子文件夹中，所有命名符合`test_*.py`或者`*_test.py`规则的文件；
...
- 清除的看到，`test_two`失败的原因是：`False = hasattr('hello', 'check')`；
